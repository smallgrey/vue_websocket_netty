<template>
    <view class="addressBook">
        ...
        <view class="list-item">
           <view v-for="(item, index) in listData" :key="index" class="section" @click="chat(item)">
...
export default {
    ...
    methods: {
            ...
            UserM.addressBook({username:this.searchData.salesmanName}).then(res => {
                this.listData = res.data.filter(item => { // 除去自己
                    if(item.headimg === "default1") {
                       item.head_img = require("../../static/img/default1.jpg")
                    }else{
                       item.head_img = require("../../static/img/default2.jpg")
                    }
                    return item.userId != this.$store.state.webSocket.user.userId
                })
            })
        },
        // 搜索事件
        ...
        chat (user) {
            uni.navigateTo({
                url: '/pages/contactMan/chatArea?user='+ JSON.stringify(user),
                success: res => {},
                fail: () => {},
                complete: () => {}
            });
